...
export function calculateHeaderGroupHeight(
  ...
  // At desktop on the product template the transparent layout is suppressed in CSS (see header.liquid),
  // so skip this workaround to avoid double-counting the header height.
  if (
...
function updateTransparentHeaderOffset() {
  ...
  // At desktop on the product template the transparent layout is suppressed, so the offset is too.
