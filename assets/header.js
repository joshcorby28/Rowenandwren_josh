...
onDocumentLoaded(() => {
  ...
  if (headerGroup) {
    // At desktop on the product template the transparent header layout is
    // suppressed in CSS (see header.liquid <style> block), so the
    // hasAttribute('transparent') check must be gated to match — otherwise
    // the header height gets double-counted.
    const desktopMql = window.matchMedia('(min-width: 750px)');
    const isHeaderTransparentActive = () => {
      if (!(header instanceof HTMLElement) || !header.hasAttribute('transparent')) return false;
      return !(document.body.classList.contains('template-product') && desktopMql.matches);
    };

    const resizeObserver = new ResizeObserver(() => {
      // Recompute the full header group height from all children rather than
      // summing only the entries in this callback batch (a ResizeObserver only
      // delivers the elements that changed size, so summing `entries` produced a
      // partial total). The transparent header's own contribution is anchored to
      // the stable --header__height (set inline by the header section) instead of
      // its live offsetHeight, which can measure ~21px taller at first paint
      // before this custom header's styles settle. This matches the inline script
      // in theme.liquid and keeps --header-group-height — and the
      // transparent-header hero sized from it — constant after load, so the hero
      // no longer jolts.
      ...
      if (isHeaderTransparentActive() && header?.parentElement?.nextElementSibling) {
        const declaredHeaderHeight = parseFloat(
          getComputedStyle(document.documentElement).getPropertyValue('--header__height')
        );
        headerGroupHeight += declaredHeaderHeight || (header instanceof HTMLElement ? header.offsetHeight : 0);
      }
    ...
    // On product, crossing the 750px breakpoint changes whether the header
    // contributes its own height to --header-group-height. ResizeObserver may
    // not fire if no element actually resized, so recompute explicitly.
    if (document.body.classList.contains('template-product')) {
      desktopMql.addEventListener('change', () => {
        const totalHeight = Array.from(headerGroup.children).reduce((sum, child) => {
          return child instanceof HTMLElement && child !== header ? sum + child.offsetHeight : sum;
        }, 0);
        const headerContribution =
          isHeaderTransparentActive() && header.parentElement?.nextElementSibling && header instanceof HTMLElement
            ? header.offsetHeight
            : 0;
        document.body.style.setProperty('--header-group-height', `${Math.round(totalHeight + headerContribution)}px`);
        document.body.style.setProperty('--transparent-header-offset-boolean', isHeaderTransparentActive() ? '1' : '0');
      });
    }
  }
});